...
void RBTree::MinHeights() {
    ...
    while (!nodes.empty()) {
        ...
        if (curr->left != nullptr && curr->right != nullptr) {
            curr->minHeight = min(curr->left->height, curr->right->height);
...
int RBTree::TreeHeightRecursive(Node *curr) {
    ...
    curr->height = 1 + max(TreeHeightRecursive(curr->left), TreeHeightRecursive(curr->right));
    return curr->height;
}
...
void RBTree::readfiles() {
    ...
    if (file.is_open()) {
        ...
        while (!file.eof()) {
            ...
            Anime temp(title, stoi(id), synopsis, genres, score);
            library.Insert(temp);
            this->Insert(temp);
    ...
    cout << "Time to insert all anime and their relations: " << (duration.count() / 1000) << " seconds" << endl;
}
...
void RBTree::displayinfo(string userInput) {
    if (this->Find(userInput) == nullptr) {
        ...
        for (int i = 0; i < userAnime.related.size(); i++) {
            ...
            cout << "Genre(s): " << userAnime.related.at(i)->genre << "   Score: " << userAnime.related.at(i)->score
                 << endl;
